...
export class MunicipalityService {
  ...
  async create(data: Prisma.MunicipalityCreateInput): Promise<Municipality>{
    ...
    const existingMunicipalityWithAccent = await this.prisma.municipality.findFirst({
      where: {
        name: {
    ...
    if (existingMunicipalityWithAccent) {
      throw new NotFoundException("Ya existe un municipio con este nombre (incluyendo acentos).");
    }
